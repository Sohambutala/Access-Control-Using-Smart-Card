...
public class NewClass 
{
    ...
    static SecretKey genkey(String login_user,int password)
    {
        String st="";
        str=login_user;
        pass=password;
        System.out.println("length of ATR "+str.length());
        while(str.length()%4!=0)
        {
            str=str+"F";
        }
        System.out.println("ATR after padding "+str);
        int i;
        int temp,ans=0;
        for(i=0;i<str.length();i=i+4)
        {
            st=str.substring(i, i+4);
            temp=Integer.parseInt(st, 16);
            ans=ans+temp;
        }
        System.out.println("final computation of ATR "+ans);
        int key=pass^ans; //xor ATR with password
        System.out.println("des key value "+key);
       ...
       System.out.println("final des key "+key);
